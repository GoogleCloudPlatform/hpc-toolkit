...
This is the path you will set as `local_key` in the next step.

If the deployment machine is not the slurm controller, copy it over the network:

```shell
scp slurmctld:/etc/munge/munge.key /opt/deployments/
```

If you are deploying from the slurm controller itself, copy it locally:
